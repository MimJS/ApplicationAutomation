...
let shopCacheTs = 0;
let energyCacheTs = 0;
const SHOP_TTL_MS = 5 * 60_000; // 5 мин — КД считаются локально из purchasedAt
const ENERGY_TTL_MS = 2 * 60_000; // 2 мин — между играми трекаем локально
  ...
  try {
    ...
    axios
      ...
      .then((userRes) => {
        ...
        tg(
          `🎮 Игра [${phase}]${boostTag}\n` +
            `⏱ ${gameTime}с | ⭐ +${delta} рейтинга\n` +
            `⚡ ${currentEnergy}→${newEnergy} | 📊 ${rating} | 💰 ${money}`,
        );
      })
      .catch(() => {
        tg(
          `🎮 Игра [${phase}]${boostTag} | ⏱ ${gameTime}с | ⭐ +${delta} | ⚡ ${currentEnergy}→${newEnergy}`,
        );
      });
...
async function tick() {
  ...
  if (boostWasActive && !boostActive && boosterEndTime) {
    ...
    tg(
      `⏰ Бустер закончился! Начинается перерыв 🔄\n` +
        `⚡ Энергия: ${currentEnergy} | ⏳ До бустера: ~${minsLeft.toFixed(0)} мин`,
    );
  }
  ...
  if (boostActive) {
    if (!drainComplete) {
      ...
      if (currentEnergy === 0) {
        drainComplete = true;
        tg(
          `✅ Энергия слита до ${currentEnergy}!\n` +
            `🛒 Скупаем ВСЕ восстановления 🔥+30% (ещё ${boostMinLeft.toFixed(1)} мин)`,
        );
        await buyAllRestorations();
        return;
      }
      tg(
        `🔻 [СЛИВ] ${currentEnergy}→${currentEnergy - 1} 🔥+30%\n` +
          `🚫 Без покупок — сначала слить все 10 обычных\n` +
    ...
    tg(
      `😴 Нет энергии под бустером! 🔥+30% ещё ${boostMinLeft.toFixed(1)} мин\n` +
        `⏳ Жду пассивную рег. (+1/10мин)...`,
    );
    await sleep(15_000);
    return;
  }
  ...
  if (canBuyBooster()) {
    tg(
      `🚀 ВСЕ УСЛОВИЯ ВЫПОЛНЕНЫ! ✅\n` +
        `⚡ Энергия=${currentEnergy}/10 | Все КД готовы ✔️\n` +
        `🔥 ПОКУПАЕМ БУСТЕР!`,
    );
    ...
    if (ok) {
      ...
      boostWasActive = false;
      tg(
        `🔥 БУСТЕР АКТИВЕН! +30% на 20 мин ⏱\n` +
          `🔻 Начинаем слив 10 обычных энергий (без покупок)`,
      );
    }
    return;
  }
  ...
  if (!chocolateBoughtThisBreak && canBuy(N.CHOCOLATE)) {
    if (currentEnergy + 2 > MAX_ENERGY) {
      const reason =
        ...
        `🎮 Играю сначала, потом куплю шоколадку`;
      tg(reason);
      await playGame("пред-шоколад");
      return;
    }
    const reason =
      `🍫 Покупаю шоколадку (+2) ✔️\n` +
      `🔄 CD=40мин → готов к бустеру (~T+83)\n` +
  ...
  if (currentEnergy === 0) {
    ...
    const reason =
      `😴 Нет энергии. Жду рег. (+1/10мин)\n` +
      `💧 Вода через ${nextWaterMin.toFixed(0)} мин\n` +
      `🔒 Бустер: [${boosterBlockReasons.join("; ")}]`;
    tg(reason);
  ...
  // Проверяем: если сыграть 1 игру — хватит ли энергии к бустеру?
  const projectedAfterPlay = projected - 1;

  if (projectedAfterPlay >= MAX_ENERGY) {
    // Даже после траты 1 энергии наберём 10 к бустеру — играем
    const surplus = projectedAfterPlay - MAX_ENERGY + 1;
    const reason =
      `🎮 [ПЕРЕРЫВ] ⚡${currentEnergy} | Излишек: +${surplus}\n` +
      `📈 Прогноз (${minsLeft.toFixed(0)}мин): ${projected}/10 → после игры ${projectedAfterPlay}/10 — играю!`;
    console.log(reason);
